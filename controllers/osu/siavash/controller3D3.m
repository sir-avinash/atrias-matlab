function [eStop, u, userOut, xym, dxy] = controller3D3(q, dq, userIn)

persistent x y tp xc yc ycp xcp xcpp ycpp dxp dyp t0 RefLeg Tcp Tcpp dypr dxpr dypl dxpl sdxl sdyl sdxr sdyr dx_c dy_c x_fpc ts dx_fp dy_fp FRFL FLFR qd a4 lFp rFp
...
T=max(.2,T0*(1+min(-k_time*(abs(dy_des-dy)),-k_time*(abs(dx_des-dx)))));
...
if abs(dx-dx_des)>.6 || abs(dy-dy_des)>.8 || max(rF,lF)<f_th0
    ts=ti;
    x=0;
    y=0;
    xc=0;
...
x_fp=[x_fp0; .2*dy_des]+[k_fp_l; k_fp_h].*[dx-dx_des; dy-dy_des]+[xs;ys]...
    ...
    +kI*[clamp(.05*(x-dx_des*(ti-ts)),-.1,.1); clamp(.05*(y-dy_des*(ti-ts)),-.1,.1)]...
