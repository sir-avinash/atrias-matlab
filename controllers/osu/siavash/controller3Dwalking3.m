function [eStop, u, userOut, xym, dxy] = controller3Dwalking3(q, dq, userIn)
...
%     T0=.37-.02-.05*(ti>t_stb);
    T0=clamp(.35-.05*(dx_des-vs)*(dx_des>vs),.2,.35);
    ctrltest=userIn(7);
else
...
if abs(dx-dx_des)>(0.2*dx_des+.3) || ~walkCmd 
    ts=ti;
    x=0;
    y=0;
    xc=0;
...
x_fp=[k_fp_l_v*dx; k_fp_h_v*dy]+[k_fp_l_e; k_fp_h_e].*[dx-dx_des; dy-dy_des]+[xs;ys]...
    +[1;0].*[clamp(kI*.05*(1+dx_des)*(x-0*xc-xd),-.05,.05); clamp(2*kI*.05*(1+dy_des)*(y-0*yc-yd),-.05,.05)]*walkCmd...
...
switch state
    ...
    case 1 % WALK --------------------------------------------------------
        ...
        deltaLx=(ldev*sign(x_st)+0*(x_st>0))*(dx<dx_des)*walkCmd;
...
if (ti-t0)>=T+.1*dx_des*walkCmd*(abs(dx-dx_des)<.5*dx_des) || ((ti-t0)>T/2 && RefLegF2~=RefLeg)
    Tcppp=Tcpp;
    Tcpp=Tcp;
    Tcp=ti-t0;
    t0=ti;
